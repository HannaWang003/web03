...
<div class="half">
    ...
    $total = $Movie->count("where `sh`='1' && `ondate` between '$today' and '$ondate'");
    $div = 4;
    $pages = ceil($total / $div);
    $now = $_GET['p'] ?? 1;
    $start = ($now - 1) * $div;
    $movies = $Movie->all("where `sh`='1' && `ondate` between '$today' and '$ondate'", "order by rank limit $start,$div");
    ?>
    <h1>院線片清單</h1>
    <style>
        .movies {
            ...
            ?>
                ...
                <div class="movie">
                    ...
                    <div style="width:100%;margin:5px 0;">
                        <button onclick="location.href='?do=intro&id=<?= $m['id'] ?>'">劇情簡介</button> <button onclick="location.href='?do=order&id=<?= $m['id'] ?>'">線上訂票</button>
                    </div>
                </div> <?php
                    }
                        ?>
