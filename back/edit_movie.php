    ...
    <div id="editMovie">
        ...
        <div>
            <table class="rb" width="100%" style="padding:10px 5px;box-sizing:border-box;">
                <tr>
                    <th width="30%">片名</th>
                    <td width="70%">: <input type="text" name="movie" value="<?= $row['movie'] ?>"></td>
                </tr>
                <tr>
                    <th>分級</th>
                ...
                <tr>
                    ...
                    <td>:
                        <select name="year" id="">
                            ...
                            for ($i = 2024; $i <= 2025; $i++) {
                            ?>
                            <option value="<?= $i ?>" <?= ($i == $row['ondate'][0]) ? "selected" : "" ?>><?= $i ?>
                            </option>
                            <?php
                            }
                            ?>
                        ...
                        <select name="month" id="">
                            ...
                            for ($i = 1; $i <= 12; $i++) {
                            ?>
                            <option value="<?= $i ?>" <?= ($i == $row['ondate'][1]) ? "selected" : "" ?>><?= $i ?>
                            </option>
                            <?php
                            }
                            ?>
                        ...
                        <select name="day" id="">
                            ...
                            for ($i = 1; $i <= 31; $i++) {
                            ?>
                            <option value="<?= $i ?>" <?= ($i == $row['ondate'][2]) ? "selected" : "" ?>><?= $i ?>
                            </option>
                            <?php
                            }
                            ?>
        ...
        <div>
            <textarea name="intro" id=""
                style="width:98%;height:100px;margin:10px auto;"><?= $row['intro'] ?></textarea>
        </div>
        <div style="width:100%;" class="ct"><input type="submit" value="修改"> <input type="reset" value="重置"></div>
